...
def main(image_path):
    ...
    for name, noisy_img in noises.items():
        ...
        results.append({
            'Noise Type': name,
            'MSE (Noisy)': round(mse(original, noisy_img), 2),
            'PSNR (Noisy)': round(psnr(original, noisy_img), 2),
            'SSIM (Noisy)': round(ssim_val(original, noisy_img), 3),
            'MSE (Denoised)': round(mse(original, denoised), 2),
            'PSNR (Denoised)': round(psnr(original, denoised), 2),
            'SSIM (Denoised)': round(ssim_val(original, denoised), 3)
        })
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Image degradation, restoration, and quality metrics.')
